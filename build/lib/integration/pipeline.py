...
async def process_document_with_ocr(document_id: str, pdf_data: bytes, dms_service: Optional[DmsService] = None) -> Dict[str, Any]:
    """
    ...
    if dms_service:
        try:
            ...
            logger.warning(f"Failed to mark OCR running for document {document_id}: {e}")
...
async def process_document_with_llm(document_id: str, ocr_results: Dict[str, Any], dms_service: Optional[DmsService] = None) -> Dict[str, Any]:
    """
    ...
    if dms_service:
        try:
            ...
            logger.warning(f"Failed to mark LLM running for document {document_id}: {e}")
    ...
    if dms_service:
        try:
            ...
            logger.warning(f"Failed to mark processing done for document {document_id}: {e}")
